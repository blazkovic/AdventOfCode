...
std::string input = R"(bot 37 gives low to bot 114 and high to bot 150
...
int g_botNumber = 0;
int bot_count = 0;
int g_numberOfInstructions = 0;
int g_numberOfProcessed = 0;
...
public:
    Bot()
    {
        m_botIndex = bot_count++;
    }

    void action(std::vector<Bot> & l_botList, std::vector<int> & p_outputList)
    {
        if (m_lowBot != -1 && m_highBot != -1 && m_lowValue != -1 && m_highValue != -1)
        {
            l_botList[m_lowBot].addValue(m_lowValue);
            l_botList[m_highBot].addValue(m_highValue);
            m_lowValue = -1;
            m_highValue = -1;
            m_lowOutput = -1;
            m_highOutput = -1;
        ...
        if (m_lowOutput != -1 && m_highBot != -1 && m_lowValue != -1 && m_highValue != -1)
        {
            p_outputList[m_lowOutput] = m_lowValue;
            l_botList[m_highBot].addValue(m_highValue);
            m_lowValue = -1;
            m_highValue = -1;
            m_lowOutput = -1;
        ...
        if (m_lowBot != -1 && m_highOutput != -1 && m_lowValue != -1 && m_highValue != -1)
        {
            p_outputList[m_highOutput] = m_highValue;
            m_lowValue = -1;
            m_highValue = -1;
            m_lowOutput = -1;
            m_highOutput = -1;
        ...
        if (m_lowOutput != -1 && m_highOutput != -1 && m_lowValue != -1 && m_highValue != -1)
        {
            p_outputList[m_lowOutput] = m_lowValue;
            p_outputList[m_highOutput] = m_highValue;
            m_lowValue = -1;
...
int getHighestBotNumber()
{
    std::istringstream l_stream(input);
    std::string l_item;
    auto l_highest = 0;
    auto l_wasLastValue = false;
    while(std::getline(l_stream, l_item, ' '))
    {
        if (l_item == "value")
        {
            l_wasLastValue = true;
        ...
        if (l_item[0] >= '0' && l_item[0] <= '9')
        {
            auto l_value = std::stoi(l_item);
            if (l_value > l_highest && !l_wasLastValue)
            {
                l_highest = l_value;
            }
        }
        l_wasLastValue = false;
    }
    return l_highest+1;
}

void buildDependiencesBetweenBots(std::vector<Bot> & l_botList)
{
    std::istringstream l_stream(input);
    std::string l_item;
    while(std::getline(l_stream, l_item, '\n'))
    {
        std::istringstream l_insideStream(l_item);
        std::string l_insideItem;
        while(std::getline(l_insideStream, l_insideItem, ' '))
        {
            if (l_insideItem == "value")
            {
                std::getline(l_insideStream, l_insideItem, ' ');
                auto l_value = std::stoi(l_insideItem);
                for (auto i = 0; i < 4; i++)
                {
                    std::getline(l_insideStream, l_insideItem, ' ');
                    if (i == 3)
                    {
                        auto l_botNumber = std::stoi(l_insideItem);
                        l_botList[l_botNumber].addValue(l_value);
                    }
                }
            }
            if (l_insideItem == "bot")
            {
                ...
                const auto l_commandWords = 11;
                const auto l_lowBotPositionInCommand = 4;
                const auto l_highBotPositionInCommand = 9;
                auto l_botNumber = 0;
                auto l_lowBot = -1;
                auto l_highBot = -1;
                for (auto i = 0; i < l_commandWords; i++)
                {
                    std::getline(l_insideStream, l_insideItem, ' ');
                    if (i == 0)
                    {
                        l_botNumber = std::stoi(l_insideItem);
                    }
                    if (i == l_lowBotPositionInCommand)
                    {
                        if(l_insideItem != "output")
                        {
                            std::getline(l_insideStream, l_insideItem, ' ');
                            l_lowBot = std::stoi(l_insideItem);
                            i++;
                        }
                        if(l_insideItem == "output")
                        {
                            std::getline(l_insideStream, l_insideItem, ' ');
                            l_botList[l_botNumber].m_lowOutput = std::stoi(l_insideItem);
                            i++;
                        }
                    }
                    if (i == l_highBotPositionInCommand)
                    {
                        if(l_insideItem != "output")
                        {
                            std::getline(l_insideStream, l_insideItem, ' ');
                            l_highBot = std::stoi(l_insideItem);
                            i++;
                        }
                        if(l_insideItem == "output")
                        {
                            std::getline(l_insideStream, l_insideItem, ' ');
                            l_botList[l_botNumber].m_highOutput = std::stoi(l_insideItem);
                            i++;
                        }
                    }
                }
                l_botList[l_botNumber].m_lowBot = l_lowBot;
                l_botList[l_botNumber].m_highBot = l_highBot;
            }
        }
    }
}
...
int main()
{
    /* This is very bad piece of code because description of problem is very confusing for me */

    auto l_botCount = getHighestBotNumber();
    std::vector<Bot> l_botList(l_botCount);
    std::vector<int> l_outputList(30, -9);

    buildDependiencesBetweenBots(l_botList);
    while(g_numberOfProcessed < g_numberOfInstructions)
    {
        for(auto i = 0u; i < l_botList.size(); i++)
        {
            l_botList[i].action(l_botList, l_outputList);
            if (g_botNumber)
            {
                break;
            }
    ...
    std::cout << "0x1x2 = " << l_outputList[0]*l_outputList[1]*l_outputList[2] << std::endl;
    return 0;
}
