...
int deepFirstSearchIterative()
{
    ...
    std::vector<std::string> l_solutionsPaths;
    ...
    while(l_nodesToExplore.size())
    {
        ...
        {
            l_solutionsPaths.push_back(l_topNode.pathSoFar);
        }

        for (const auto singleNode : l_actualNodeNeighbours)
        {
            if (isInRange(singleNode) and isOpenSpace(l_topNode, singleNode) and not isDestination(l_topNode))
            {
                l_nodesToExplore.push(singleNode);
            }
        }
    }
    std::sort(l_solutionsPaths.begin(), l_solutionsPaths.end(), [](const std::string & p_first, const std::string & p_second){ return p_first.size() > p_second.size(); });
    return l_solutionsPaths[0].size();
}

int main()
{
    std::cout << "Longest path = " << deepFirstSearchIterative() << std::endl;
...
Your puzzle input is still qzthpkfp.
