...
{
    auto l_charIndex = p_stringToScramble.find(p_item[35]);
    if (l_charIndex > 3) l_charIndex++;
    l_charIndex++;
    while(l_charIndex)
    {
        const auto l_temp = p_stringToScramble.substr(p_stringToScramble.size() - 1);
...
{
    const auto l_xIndex = std::stoul(p_item.substr(14));
    const auto l_yIndex = std::stoul(p_item.substr(28));
...
void parseAndExecute(const std::string & p_item, std::string & p_stringToScramble)
{
    ...
    if (p_item.find("rotate left ") != std::string::npos)                           rotateLeft(p_item, p_stringToScramble);
    if (p_item.find("rotate right ") != std::string::npos)                          rotateRight(p_item, p_stringToScramble);
...
int main()
{
    std::string l_stringToScramble("abcdefgh");
    std::istringstream l_input(g_input);
    std::string l_item;
    while(getline(l_input, l_item, '\n'))
    {
        parseAndExecute(l_item, l_stringToScramble);
    }
