...
void removeChecksum(std::string & p_input)
{
    p_input.erase(std::find(p_input.begin(), p_input.end(), '['), p_input.end());
}

void parseStringToVector(const std::string & p_input, std::vector<std::string> & p_vector)
{
    std::istringstream l_input(p_input);
    std::string l_item;
    while(getline(l_input, l_item, ']'))
    {
        removeChecksum(l_item);
...
int main()
{
    ...
    std::vector<std::string> l_outputStrings;
    parseStringToVector(input, l_inputStrings);

    auto l_northStoreId = 0;
    auto l_sectorId = 0;
    for(auto & it : l_inputStrings)
    {
        l_sectorId = std::stoi(it.substr(it.size()-3, it.size()));
        it.erase(it.size()-3, it.size());
        std::string l_line;
        for (auto & singleChar : it)
        {
            if (singleChar != '-')
            {
                for(auto i = l_sectorId; i > 0; i--)
                {
                    singleChar++;
                    if (singleChar > 'z')
                    {
        ...
        l_outputStrings.push_back(l_line);
        if (l_outputStrings.back().find("northpole") != std::string::npos)
        {
            std::cout << l_outputStrings.back() << std::endl;
            l_northStoreId = l_sectorId;
        }
    }
    std::cout << "North store sector ID = " << l_northStoreId << std::endl;
    return 0;
}
