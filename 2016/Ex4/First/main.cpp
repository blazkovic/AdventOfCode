...
std::string input = R"(nzydfxpc-rclop-qwzhpc-qtylyntyr-769[oshgk]
...
void removeDash(std::string & p_input)
{
    p_input.erase(remove_if(p_input.begin(), p_input.end(), [](char x){return x=='-';}), p_input.end());
}

std::string removeToCharacter(std::string p_input, char p_char)
{
    return p_input.erase(0, p_input.find(p_char) + 1);
}

void parseStringToVector(std::string & p_input, std::vector<std::string> & p_vector)
{
...
bool isChecksumValid(const std::vector<std::pair<char, int>> & p_sortedChecksum, const std::string & p_checksum)
{
    for (auto i = 0; i<5; i++)
    {
        if (p_sortedChecksum[i].first != p_checksum[i]) return false;
    }
    return true;
}

bool isRealRoom(const std::map<char, int> & p_charCounter, const std::string & p_checksum)
{
    std::vector<std::pair<char, int>> l_sortedChecksum(p_charCounter.begin(), p_charCounter.end());

    auto l_pairCompare = [](const auto & p_left, const auto & p_right)
    {
        if (p_left.second > p_right.second) return true;
        else if (p_left.second == p_right.second)
        {
            return p_left.first < p_right.first;
        }
        return false;
    };
    std::sort(l_sortedChecksum.begin(), l_sortedChecksum.end(), l_pairCompare);

    return isChecksumValid(l_sortedChecksum, p_checksum);
}
...
int main()
{
    removeSpaces(input);
    removeDash(input);
    std::vector<std::string> l_inputStrings;
    parseStringToVector(input, l_inputStrings);

    auto l_sumOfRealSectorIds = 0;
    std::map<char, int> l_charCounter;
    std::string l_sectorId;
    std::string l_checksum;

    for(auto & it : l_inputStrings)
    {
        l_charCounter.clear();
        l_sectorId.clear();

        for(auto & singleChar : it)
        {
            if (std::isdigit(singleChar))
            {
                l_sectorId += singleChar;
            }
            else if (singleChar == '[')
            {
                l_checksum = removeToCharacter(it, singleChar);
            }
            else
            {
                l_charCounter[singleChar]++;
            }
        }
        if (isRealRoom(l_charCounter, l_checksum))
        {
            l_sumOfRealSectorIds += std::stoul(l_sectorId);
        }
    }
    std::cout << l_sumOfRealSectorIds << std::endl;
