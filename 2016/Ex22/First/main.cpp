...
std::string g_input = R"(/dev/grid/node-x0-y0     85T   68T    17T   80%
...
{
public:
    Node(int p_size, int p_used)
        : m_size(p_size), m_used(p_used), m_avaiable(p_size-p_used)
    {}

    int getAvaiable() const
    {
        return m_avaiable;
    }

    bool isPair(const Node & p_node)
    {
        if (m_used == 0) return false;
        if (m_used <= p_node.getAvaiable()) return true;
        return false;
    }
...
private:
    const int m_size;
    const int m_used;
    const int m_avaiable;
};

void createNode(std::vector<Node> & p_nodes, const std::string & p_input)
{
    ...
    {
        if (std::isdigit(p_input[i]))
        {
            l_x+=p_input[i];
        }
    }
    for (auto i = 18u; i < 22; i++)
    {
        if (std::isdigit(p_input[i]))
        {
            l_y+=p_input[i];
    ...
    p_nodes.push_back({std::stoul(l_size), std::stoul(l_used)});
}

int checkNumberOfPairs(std::vector<Node> p_nodes)
{
    auto l_numOfPairs = 0;
    for (auto i = 0; i < p_nodes.size() - 1; i++)
    {
        for (auto j = i; j < p_nodes.size(); j++)
        {
            if (p_nodes[i].isPair(p_nodes[j]))
            {
                l_numOfPairs++;
            }
            if (p_nodes[j].isPair(p_nodes[i]))
            {
                l_numOfPairs++;
            }
        }
...
int main()
{
    ...
    {
        createNode(l_nodes, l_item);
    }

    const auto l_result = checkNumberOfPairs(l_nodes);

    std::cout << "Number of viable pairs of nodes = " << l_result << std::endl;
    return 0;
}
