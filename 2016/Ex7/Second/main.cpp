...
std::string input =
        R"(xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
...
using BracketPair = std::pair<bool, std::string>;
using Splited = std::vector<BracketPair>;
...
void splitToPairs(const std::string & p_line, Splited & p_splited)
{
    for (auto i = 0u; i < p_line.size();)
    {
        if (p_line[i] == '[')
        {
            auto l_rightBracket = p_line.find(']', i);
            p_splited.push_back(std::make_pair(true, std::string(p_line.substr(i + 1, l_rightBracket - i - 1))));
            i = l_rightBracket+1;
        }
        else
        {
            auto l_leftBracket = p_line.find('[', i);
            if (l_leftBracket != std::string::npos)
            {
                p_splited.push_back(std::make_pair(false, std::string(p_line.substr(i, l_leftBracket - i))));
                i = l_leftBracket;
            }
            else
            {
                p_splited.push_back(std::make_pair(false, std::string(p_line.substr(i, p_line.size()))));
                i = p_line.size();
            }
        }
    }
...
void createSSLSequenceVector(std::vector<std::string> & p_inside, const std::string & p_string)
{
    for (auto i = 0u; i < p_string.size() - 2; i++)
    {
        auto l_isFirstAndThirdTheSame = p_string[i] == p_string[i+2];
        auto l_isFirstAndSecondDifferent = p_string[i] != p_string[i+1];
        if (l_isFirstAndThirdTheSame and l_isFirstAndSecondDifferent)
        {
            p_inside.push_back(std::string(p_string.substr(i, 3)));
        }
    }
}

std::string sslTransformation(const std::string & p_string)
{
    std::string l_newString(p_string);
    l_newString[0] = l_newString[1];
    l_newString[1] = l_newString[2];
    l_newString[2] = l_newString[0];
    return l_newString;
}

bool checkIfValidSSLString(std::vector<std::string> l_first, std::vector<std::string> l_second)
{
    for (const auto & it : l_first)
    {
        auto l_found = std::find(l_second.begin(), l_second.end(), it);
        if (l_found != l_second.end()) return true;
    }
    return false;
}

bool isValid(const Splited & p_splited)
{
    auto l_isValid = false;

    std::vector<std::string> l_inside;
    std::vector<std::string> l_outside;

    for (const auto & it : p_splited)
    {
        auto & l_secondElement = it.second;
        if (it.first)
        {
            createSSLSequenceVector(l_inside, l_secondElement);
        }
        else
        {
            createSSLSequenceVector(l_outside, l_secondElement);
        }
    }

    std::transform(l_inside.begin(), l_inside.end(), l_inside.begin(),
                   [](const std::string & p_string) { return sslTransformation(p_string); });

    l_isValid = checkIfValidSSLString(l_inside, l_outside);
...
int main()
{
    ...
    auto l_numberOfTLSSupportedIPs = 0;
    std::istringstream l_stream(input);
    std::string l_item;
    Splited l_splited;
    while(std::getline(l_stream, l_item))
    {
        splitToPairs(l_item, l_splited);
        if (isValid(l_splited))
        {
            l_numberOfTLSSupportedIPs++;
        }
        l_splited.clear();
    }
    std::cout << "Number of TLS supported IPs = " << l_numberOfTLSSupportedIPs << std::endl;
