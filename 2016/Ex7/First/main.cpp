...
bool isABBA(const std::string & p_string)
{
    for (auto i = 0u; i < p_string.size() - 3; i++)
    {
        auto l_isFirstAndFourthTheSame = p_string[i] == p_string[i+3];
        auto l_isSecondAndThirdTheSame = p_string[i+1] == p_string[i+2];
        auto l_isPairDifferent = p_string[i] != p_string[i+1];
        if (l_isFirstAndFourthTheSame and l_isSecondAndThirdTheSame and l_isPairDifferent)
        {
            return true;
        }
    }
...
bool isValid(const Splited & p_splited)
{
    ...
    {
        auto & l_firstElement = it.first;
        auto & l_secondElement = it.second;

        if (l_firstElement and isABBA(l_secondElement))
        {
            return false;
        }
        if (!l_firstElement and isABBA(l_secondElement))
