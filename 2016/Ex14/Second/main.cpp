...
std::string hashStretching(const MD5 & p_md5Hasher, const std::string & p_hash)
{
    std::string l_output = p_hash;
    for (auto i = 0u; i < 2016; i++)
    {
        l_output = p_md5Hasher.getHash(l_output);
    }
    return l_output;
}
...
int main()
{
    ...
    while (l_numberOfFoundKeys < 64)
    {
        if (l_vectorOfHashes.size() == l_currentIndex)
        {
            ...
            l_vectorOfHashes.push_back(hashStretching(l_md5Hasher, l_hash));
        }
