...
int main()
{
    ...
    {
        std::istringstream l_stream1(l_item);
        std::string l_item1;
        std::string registerToChange{};
        bool add = false;
        int valueToAdd = 0;
        std::string registerToCompare{};
        std::string operationToCompare{};
        int valueToCompare = 0;
        int i = 0;
        while(getline(l_stream1, l_item1, ' '))
        {
            if (i == 0) registerToChange = l_item1;
            if (i == 1)
            {
                if (l_item1 == "inc")
                {
            ...
            if (i == 2) valueToAdd = std::atoi(l_item1.c_str());
            if (i == 4) registerToCompare = l_item1;
            if (i == 5) operationToCompare = l_item1;
            if (i == 6) valueToCompare = std::atoi(l_item1.c_str());
            ++i;
        }
        if (operationToCompare == "==")
        {
            if (registers[registerToCompare] == valueToCompare)
            {
                if (add)
                {
                    registers[registerToChange] += valueToAdd;
                }
                else
                {
                    registers[registerToChange] -= valueToAdd;
        ...
        if (registers[registerToChange] > maxValueEver) maxValueEver = registers[registerToChange];
    }


    std::cout << "Max value ever in register = " << maxValueEver << std::endl;
    return 0;
}
...
std::string input1 = R"(b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10)";

std::string input = R"(d dec 461 if oiy <= 1
