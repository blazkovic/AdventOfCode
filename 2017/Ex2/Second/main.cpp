...
std::string input = R"(737	1866	1565	1452	1908	1874	232	1928	201	241	922	281	1651	1740	1012	1001
...
int main()
{
    ...
    {
        std::istringstream l_line(l_item);
        std::string l_value;
        std::set<int, std::greater<int>> oneRow;
        while(getline(l_line, l_value, '\t'))
        {
            const auto currentValue = std::stoi(l_value);
            oneRow.insert(currentValue);
        }
        for(auto it = oneRow.begin(); it != oneRow.end(); ++it)
        {
            for(auto innerIt = it;;)
            {
                ++innerIt;
                if (innerIt == oneRow.end()) break;
                const auto isDivisible = static_cast<double>(*it) / *innerIt;
                if (isDivisible == std::floor(isDivisible)) checksum += isDivisible;
            }
        }
    }
