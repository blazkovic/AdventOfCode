...
constexpr auto input = R"(sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
bool checkIfUniquePassword(std::vector<std::map<char, int>> & words)
{
...
int main()
{
    ...
    while(getline(l_stream, l_item, '\n'))
    {
        ...
        while(getline(l_line, l_value, ' '))
        {
            ...
            for(const auto & it : l_value)
            {
                ++characters[it];
            }
            words.push_back(characters);
        }
        if (checkIfUniquePassword(words))
        {
            ++correctPasswords;
        }
    }
    std::cout << "correctPasswords = " << correctPasswords << std::endl;
