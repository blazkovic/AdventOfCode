...
void swapRange(std::vector<int> & originalList, const unsigned currentIndex, const unsigned numOfElements)
{
    ...
    if (secondElement >= originalList.size()) secondElement = secondElement % originalList.size();
...
int main()
{
    ...
    auto skipSize = 0;
    for(auto i = 0u, currentIndex = 0u; i < lengths.size(); ++i, ++skipSize)
    {
    ...
    std::cout << "Multiplying the first two numbers in the list = " << originalList[0] * originalList[1] << std::endl;
    return 0;
}
