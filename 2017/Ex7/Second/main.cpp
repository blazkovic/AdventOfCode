...
std::string findRootName(const std::string & input)
{
    ...
    {
        std::string name{};
        while (isLetter(input[i]) and i < input.size())
        {
            name += input[i];
            ++i;
        }
...
void addChildrens(Node & root, const std::string & input)
{
    ...
    while(getline(l_stream, l_item, '\n'))
    {
        ...
        {
            if (l_item1 != root.name && i == 0) break;
            if (i == 1)
            {
                std::string value(std::begin(l_item1) + 1, std::end(l_item1) - 1);
                root.weight = std::atoi(value.c_str());
            }
            else if (i > 2)
            {
                if(*(std::end(l_item1) - 1)  == ',')
                {
                    child.name = std::string(std::begin(l_item1), std::end(l_item1) - 1);
                }
                else
                {
                    child.name = l_item1;
                ...
                addChildrens(*(root.childrens.end()-1), input);
            }
            ++i;
        }
    }
    for (const auto & it : root.childrens)
    {
...
{
    ++recursionDeep;
    std::cout << root.name << " " << root.weight << " num of childrens " << root.childrens.size()
              << " Total weight = " << root.childrenWeights + root.weight << std::endl;
...
std::string input = R"(navfz (187) -> jviwcde, wfwor, vpfabxa
adtfnx (74)
endsg (33)
